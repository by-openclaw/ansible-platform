...
| `ciso-assistant.yml` | CISO Assistant — the compliance registry named by ADR security/0002 (ISO 27001:2022, NIS2, GDPR catalogs, applied controls, evidence linking back to the ADRs). Two pinned containers plus a task worker on lxc-grc-01, database on the shared cluster, evidence in SeaweedFS S3, Traefik route (private) and Authentik. One manual step by upstream's design: bind OIDC in its own admin UI and create the users first. |
