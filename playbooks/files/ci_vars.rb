# Reconcile GitLab INSTANCE-level CI/CD variables (generic; used for Harbor and npm) so every
# pipeline sees them. Idempotent: writes only on diff,
# prints one line per change, ends with `changes=N` + CI_VARS_DONE.
# Desired state comes from $CI_VARS_DESIRED (JSON) so the token never touches
...
# Run via: CI_VARS_DESIRED='{...}' gitlab-rails runner harbor_ci_vars.rb
require "json"

src = ENV["CI_VARS_DESIRED"]
