...
| **wazuh** | **A** | `wazuh-indexer-data` (alerts, FIM history, vulnerability inventory), manager volumes (agent keys, groups) | PBS guest image; secrets in Vault `prod/wazuh/*`; internal PKI regenerable | play + volume restore |
