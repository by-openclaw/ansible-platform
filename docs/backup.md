...
| **mailcow** | **B** | vmail, MariaDB, redis, conf | `mailcow-backup.timer` **01:00** → helper `backup all` → `/var/backups/mailcow` (→ PBS 01:30); S3 copy via `--tags backup` | `helper-scripts/backup_and_restore.sh restore` |
...
| **verdaccio** | **A** | `/opt/verdaccio/storage` (local v1) | PBS guest image | play + restore dir *(S3 storage = follow-up)* |
